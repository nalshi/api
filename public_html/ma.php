<?php
// ========================================================================
// ملف نقل المنتجات من Cloudflare D1 إلى قاعدة بيانات TiDB Cloud
...
try {
    // 1. إنشاء جدول المنتجات (products) في TiDB Cloud بالتنسيق الصحيح إن لم يكن متواجداً
    ...
    echo "1. تم فحص وتهيئة جدول المنتجات (products) بنجاح.\n";

    // 2. جلب جميع المنتجات من Cloudflare D1
    echo "2. جاري الاتصال بـ Cloudflare D1 لسحب المنتجات الحالية...\n";
    $d1_products = fetch_from_d1("SELECT * FROM products");
    $count = count($d1_products);
    echo "-> تم العثور على ($count) منتج داخل Cloudflare D1.\n\n";

    if ($count === 0) {
        die("انتهى: لا توجد منتجات مسجلة في Cloudflare D1 لنقلها.\n");
    }

    // 3. تجهيز جملة الإدخال مع خاصية التحديث عند التكرار (ON DUPLICATE KEY UPDATE)
    ...
    foreach ($d1_products as $row) {
        // معالجة حقل الخيارات options للتأكد من تخزينه كـ JSON صالح
        $options = $row['options'] ?? '[]';
