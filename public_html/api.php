...
$exempted_actions = [
    ...
    'auth_request_otp', 
    'auth_verify_otp', 
    'check_customer_session', // سنعالجه داخلياً ليدعم التوكن
    'login', 
    'check_phone', 
    'register_init', 
    'register_verify'
...
$customer_jwt_token = generate_signed_token($payload_token, 43200); // 30 days
    ...
    if ($target_id) {
        ...
        if ($customer_data && $customer_data['is_active'] == 1) {
...
$stmt_cust = $pdo->prepare("SELECT full_name, address, phone FROM customers WHERE id = ?");
 ...
 $actual_cart_total = 0;
                ...
                foreach ($cart_items as &$c_item) {
                    ...
                    // ⭐ التعديل الأمني والجذري هنا: جلب merchant_id الحقيقي من الداتابيز
                    $stmt_check_price = $pdo->prepare(
                        "SELECT l.merchant_price, l.merchant_id, p.discount 
                    ...
                    if ($p_data) {
                        // فرض المعرف الصحيح للتاجر بالقوة من قاعدة البيانات
                        ...
                        // تجميع المنتجات بناءً على المعرف الحقيقي والموثوق
                ...
                foreach ($grouped_by_merchant as $merchant_id => $items) {
                    ...
                    foreach ($items as $item) {
                     ...
                     if (!$listing) throw new Exception("المنتج {$item['name']} لم يعد متوفراً.");
                        ...
                        // ⭐ أخذ العملة الحقيقية من المنتج وتسجيلها في الفاتورة
                    ...
                    // جلب بيانات التاجر والإعدادات لمعرفة هل التوصيل مجاني
                    ...
                    // حساب رسوم التوصيل بناءً على إعدادات التاجر
              ...
              // إذا كانت عملة المنتجات يمني، نجمعها مع التوصيل. إذا كانت سعودي/دولار، لا نجمعها ونترك التوصيل منفصلاً
                    ...
                    $ticket_payload =[
                        ...
                        'financials' =>[
                            ...
                            'currency' => $currency,
                            'delivery_currency' => 'YER' // إجبار رسوم التوصيل لتكون باليمني دائماً
                        ],
 ...
 // ==============================================================
// ⭐ تحسين إرسال جرس التنبيه للتاجر
// ==============================================================
...
curl_setopt($ch_fb, CURLOPT_CUSTOMREQUEST, "PATCH"); // استخدام PATCH بدلاً من PUT للحفاظ على البيانات الأخرى
curl_setopt($ch_fb, CURLOPT_POSTFIELDS, json_encode([
    'new_order' => [
        'timestamp' => time(),
        'order_id' => $ticket_id,
        'random_sync' => rand(1000, 9999) // رقم عشوائي لضمان أن الفايربيس يرى التغيير دائماً
    ]
]));
curl_setopt($ch_fb, CURLOPT_RETURNTRANSFER, true);
...
if (!defined('APP_SECRET_KEY')) define('APP_SECRET_KEY', 'nalsh_fallback_secret_9988');
$token = generate_signed_token($payload, 480); // 8 hours
...
case 'save_merchant_settings':
    ...
    // ⭐ كشف الخطأ الحقيقي بدون أي إخفاء
    $msg = $e->getMessage();
    $line = $e->getLine();
    ...
    if (strpos($msg, 'REQUIRE_PROFILE_UPDATE:') !== false) {
        ...
        $code = 500;
        // هنا سيظهر لك الخطأ الفعلي (مثل اسم الجدول الناقص أو المشكلة في قاعدة البيانات)
        $msg = "تفاصيل الخطأ: " . $msg . " (في السطر: " . $line . ")";
    }
