...
function sync_smart_store_structure($pdo, $merchant_id) {
    try {
        ...
        // ✅ التعديل هنا: إضافة l.id as listing_id
        $stmt_featured = $pdo->query("
            SELECT p.id as global_product_id, p.name, p.image, p.department, p.discount, p.category_id,
                   l.id as listing_id, l.merchant_price as price, l.currency, u.id as merchant_id, u.username as merchant_username, u.store_name as merchant_name
...
// =======================================================
// ⭐ النظام المعماري الجديد: إنشاء التحديثات الجزئية (Delta Sync)
// =======================================================
function publish_delta_update($pdo, $product_id, $action_type = 'update') {
    // إنشاء مجلد cdn إذا لم يكن موجوداً
    $cdn_dir = __DIR__ . '/cdn/';
    if (!is_dir($cdn_dir)) {
    ...
    // إذا كان الحدث إضافة أو تعديل، نجلب بيانات المنتج كاملة ليتعرف عليها العميل
    if ($action_type !== 'delete') {
        $stmt = $pdo->prepare("
            SELECT p.id as global_product_id, p.name, p.mainDescription as description, p.image, p.sizes as options, p.discount, p.department, p.category_id,
                   l.id as listing_id, l.merchant_price as price, l.quantity, l.quantity_type, l.currency, l.is_available,
                   c.name as type, u.id as merchant_id, u.username as merchant_username, u.store_name as merchant_name
            FROM merchant_listings l
            JOIN products p ON l.global_product_id = p.id
            JOIN users u ON l.merchant_id = u.id
            LEFT JOIN categories c ON p.category_id = c.id
        ...
        if ($product) {
            ...
            $delta_data['product'] = $product;
        }
    }

    // 1. كتابة التعديل في ملف delta.json
    file_put_contents($cdn_dir . 'delta.json', json_encode([$delta_data], JSON_UNESCAPED_UNICODE));
    
    // 2. تحديث ملف version.txt (هذا هو المفتاح الذي يراقبه ملايين العملاء)
    file_put_contents($cdn_dir . 'version.txt', time());
}
// =======================================================
// ⭐ نظام الإرسال إلى Cloudflare (الدرع العالمي)
// =======================================================
function publish_to_cloudflare($key, $value) {
    $worker_url = 'https://nalsh-sync.nasermsasalsh.workers.dev/update';
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 5); // لا ينتظر أكثر من 5 ثواني لضمان سرعة التاجر
    $res = curl_exec($ch);
    curl_close($ch);
    return $res;
}
...
function sync_store_update($pdo, $product_id, $action = 'update') {
    $new_version = time();
    
    // 1. تحديث رقم الإصدار (ليعرف الملايين أن هناك جديد)
    publish_to_cloudflare('version', ['v' => $new_version]);

    // 2. إذا كان الحدث ليس حذفاً، نرسل بيانات المنتج المحدثة
    if ($action !== 'delete') {
        $stmt = $pdo->prepare("SELECT p.id, p.name, l.merchant_price as price, p.image, l.currency, l.listing_id FROM products p JOIN merchant_listings l ON p.id = l.global_product_id WHERE p.id = ? LIMIT 1");
        $stmt->execute([$product_id]);
        $prod = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($prod) {
            publish_to_cloudflare('delta', ['action' => 'update', 'product' => $prod]);
        }
    } else {
        publish_to_cloudflare('delta', ['action' => 'delete', 'product_id' => $product_id]);
    }
}
    // =======================================================
