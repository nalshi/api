...
$fb_url = getenv('FIREBASE_DB_URL') ?: $_ENV['FIREBASE_DB_URL'] ?? '';
$fb_secret = getenv('FIREBASE_DB_SECRET') ?: $_ENV['FIREBASE_DB_SECRET'] ?? '';

define('FIREBASE_DB_URL', $fb_url);
define('FIREBASE_DB_SECRET', $fb_secret);
...
function sync_to_firebase($merchant_username, $node, $item_id, $data, $method = 'PUT') {
    ...
    if (empty($fb_secret)) return; // تفادي الأخطاء إذا لم يتم وضع الرقم السري
...
$exempted_actions = [
    ...
    'recover_init', 'recover_check_otp', 'recover_set_password','build_cache_cron','force_sync_to_firebase'
];
// قراءة المدخلات
$input = [];
if (!empty(file_get_contents('php://input'))) {
...
case 'force_sync_to_firebase':
            ...
            if (!empty($fb_orders)) {
                $secret_path = get_firebase_secret_path($user_id, $m_username);
                sync_to_firebase($m_username, "orders_secured/" . $secret_path . "/active_orders", null, $fb_orders, 'PUT');
            }
...
// 🚀 [تعديل Firebase]: تحديث الحالة
  ...
  case 'merchant_cancel_order':
            ...
            try {
                ...
                $pdo->commit();
// 🚀 [تعديل Firebase]: نقل الطلب للمؤرشفة
                $ticket_data['id'] = $ticket_id;
                $ticket_data['status'] = 'completed';
                sync_to_firebase($_SESSION['username'], 'archived_orders', $ticket_id, $ticket_data, 'PUT');
                sync_to_firebase($_SESSION['username'], 'active_orders', $ticket_id, null, 'DELETE');                
                // إرسال إشعار لحظي
