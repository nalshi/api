...
function sync_to_firebase($merchant_username, $node, $item_id, $data, $method = 'PUT') {
    // التأكد من وجود إعدادات فايربيس قبل الإرسال
    if (empty(FIREBASE_DB_URL) || empty(FIREBASE_DB_SECRET)) {
        error_log("Firebase Sync Failed: Missing Environment Variables in Render.");
        return;
    }

    // تشفير اسم المستخدم ليكون صالحاً كمسار في JSON
    $safe_username = preg_replace('/[^a-zA-Z0-9_]/', '_', $merchant_username);
    
    // بناء المسار المعزول والمحمي بالرقم السري
    $url = FIREBASE_DB_URL . "stores/" . $safe_username . "/" . $node . "/" . $item_id . ".json?auth=" . FIREBASE_DB_SECRET;
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 2); // تنفيذ سريع في الخلفية كي لا يعطل الـ API
    curl_exec($ch);
    curl_close($ch);
}
