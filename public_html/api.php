...
if (empty($m_username)) {
    $stmt_u = $pdo->prepare("SELECT username FROM users WHERE id = ?");
    $stmt_u->execute([$user_id]);
    $m_username = $stmt_u->fetchColumn();
}
$ticket_data['id'] = $order_id; // 👈 المتغير الصحيح هو order_id
$ticket_data['status'] = 'cancelled'; // 👈 الحالة الصحيحة هي ملغي
sync_to_firebase($m_username, 'archived_orders', $order_id, $ticket_data, 'PUT');
sync_to_firebase($m_username, 'active_orders', $order_id, null, 'DELETE');      
                // إرسال إشعار لحظي
