...
function sync_to_firebase($merchant_username, $node, $item_id, $data, $method = 'PUT') {
    // ⚠️ جلب الرابط والرقم السري من إعدادات Render
    $fb_url = getenv('FIREBASE_DB_URL') ?: $_ENV['FIREBASE_DB_URL'] ?: 'https://shiban-a2757-default-rtdb.europe-west1.firebasedatabase.app/';
    $fb_secret = getenv('FIREBASE_DB_SECRET') ?: $_ENV['FIREBASE_DB_SECRET'] ?: ''; 
    ...
    // إذا كان item_id فارغ، معناه نحدث المجلد بالكامل (مثل الإعدادات)
...
// 🚀 [تعديل Firebase]: إرسال الطلب الجديد لمسار التاجر
                        // 🚀 [تعديل Firebase]: إرسال الطلب الجديد لمسار التاجر
$stmt_username = $pdo->prepare("SELECT username FROM users WHERE id = ?");
$stmt_username->execute([$merchant_id]);
$m_username = $stmt_username->fetchColumn();

$ticket_id_for_alert = $ticket_id; // 👈 يجب تعيين الـ ID أولاً قبل استخدامه!

$fb_order_data = json_decode($json_data, true);
$fb_order_data['id'] = $ticket_id_for_alert;
$fb_order_data['status'] = $final_status;
$fb_order_data['created_at'] = date('Y-m-d H:i:s');

sync_to_firebase($m_username, 'active_orders', $ticket_id_for_alert, $fb_order_data, 'PUT');
                    }
...
case 'login':
            ...
            if (count($valid_logins) === 1) {
              ...
              // 1. تعريف المفتاح السري أولاً لتجنب الأخطاء
                if (!defined('APP_SECRET_KEY')) define('APP_SECRET_KEY', 'nalsh_fallback_secret_9988');

                // 2. إغلاق المصفوفة بشكل صحيح
                $payload = [
                    'user_id' => $user['id'], 
...
// توليد اسم مجلد سري ومعقد للطلبات خاص بهذا التاجر فقط
$payload['firebase_secret_path'] = md5($user['id'] . 'SUPER_SECRET_KEY_123' . $user['username']);                
                // 3. إضافة المتغير الجديد خارج أقواس المصفوفة
...
// 🚀 [تعديل Firebase]: مزامنة المنتج المضاف/المعدل
        $merchant_username = $_SESSION['username'] ?? '';
        if (empty($merchant_username) && $user_id) {
            $stmt_u = $pdo->prepare("SELECT username FROM users WHERE id = ?");
            $stmt_u->execute([$user_id]);
            $merchant_username = $stmt_u->fetchColumn();
        }
        
        if ($updated_product && !empty($merchant_username)) {
            // تحديث في فايربيس
            sync_to_firebase($merchant_username, 'products', $target_product_id, $updated_product, 'PUT');
        }
        send_response('success', $response_data);
...
case 'force_sync_to_firebase':
            ...
            // 2. رفع كل المنتجات الخاصة بهذا التاجر
            ...
            if (!empty($fb_orders)) {
                // ⭐ التعديل هنا: توحيد المسار ليكون مباشرة في active_orders لكي تقرأه اللوحة
                sync_to_firebase($m_username, "active_orders", null, $fb_orders, 'PUT');
            }
...
case 'list_products':
        ...
        case 'get_product':
            ...
            try {
                ...
                $pdo->commit();
// 🚀 [تعديل Firebase]: مسح المنتج من فايربيس
                sync_to_firebase($prod_info['username'], 'products', $product_id, null, 'DELETE');                
...
// 🚀 [تعديل Firebase]: تحديث الحالة
                ...
                if ($updated_p) {
                    ...
                    sync_to_firebase($updated_p['merchant_username'], 'products', $pid, $updated_p, 'PUT');
                }     
...
flag_cache_for_rebuild($user_id ?? null); }
        ...
        case 'process_sale':
            ...
            try {
                $pdo->beginTransaction();
                $sql_listing = "SELECT l.*, p.name as product_name, p.discount, u.username as merchant_username FROM merchant_listings l JOIN products p ON l.global_product_id = p.id JOIN users u ON l.merchant_id = u.id WHERE l.global_product_id = ? AND l.merchant_id = ? FOR UPDATE";
                ...
                // 🚀 [المزامنة اللحظية مع Firebase بعد البيع]
                $stmt_get = $pdo->prepare("SELECT p.id as global_product_id, p.name, p.mainDescription as description, p.image, p.sizes as options, p.discount, p.department, p.category_id, l.id as listing_id, l.merchant_price as price, l.quantity, l.quantity_type, l.currency, c.name as type, u.id as merchant_id, u.username as merchant_username, u.store_name as merchant_name FROM merchant_listings l JOIN products p ON l.global_product_id = p.id JOIN users u ON l.merchant_id = u.id LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = ? LIMIT 1");
                $stmt_get->execute([$pid]);
                $updated_p = $stmt_get->fetch(PDO::FETCH_ASSOC);
                if ($updated_p) {
                    $updated_p['options'] = json_decode($updated_p['options'] ?? '[]', true) ?:[];
                    sync_to_firebase($listing['merchant_username'], 'products', $pid, $updated_p, 'PUT');
                }
      ...
      case 'process_return':
            ...
            try {
                $pdo->beginTransaction();
                $stmt = $pdo->prepare("SELECT s.*, u.username as merchant_username FROM sales_log s JOIN users u ON s.user_id = u.id WHERE s.id = ? AND s.type = 'sale' FOR UPDATE"); $stmt->execute([$saleId]); $orig_sale = $stmt->fetch(PDO::FETCH_ASSOC); if(!$orig_sale) throw new Exception('الفاتورة الأصلية غير موجودة');
                ...
                // 🚀 [المزامنة اللحظية مع Firebase بعد الاسترجاع]
                $pid = $orig_sale['product_id'];
                $stmt_get = $pdo->prepare("SELECT p.id as global_product_id, p.name, p.mainDescription as description, p.image, p.sizes as options, p.discount, p.department, p.category_id, l.id as listing_id, l.merchant_price as price, l.quantity, l.quantity_type, l.currency, c.name as type, u.id as merchant_id, u.username as merchant_username, u.store_name as merchant_name FROM merchant_listings l JOIN products p ON l.global_product_id = p.id JOIN users u ON l.merchant_id = u.id LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = ? LIMIT 1");
                $stmt_get->execute([$pid]);
                $updated_p = $stmt_get->fetch(PDO::FETCH_ASSOC);
                if ($updated_p) {
                    $updated_p['options'] = json_decode($updated_p['options'] ?? '[]', true) ?:[];
                    sync_to_firebase($orig_sale['merchant_username'], 'products', $pid, $updated_p, 'PUT');
                }
...
// 🚀 [تعديل Firebase]: تحديث الحالة
                ...
                sync_to_firebase($_SESSION['username'], 'active_orders', $order_id, $fb_data, 'PUT');            
      ...
      case 'merchant_update_order_status':
            ...
            if ($stmt->rowCount() > 0) {
                // 🚀 إضافة التحديث اللحظي لـ Firebase
                ...
                if($o_data) {
                    ...
                    $m_username = $_SESSION['username'] ?? $pdo->query("SELECT username FROM users WHERE id = $user_id")->fetchColumn();
                    sync_to_firebase($m_username, 'active_orders', $order_id, $fb_data, 'PUT');
                }
        ...
        case 'merchant_confirm_delivery_code':
            ...
            try {
                ...
                // 🚀 إضافة التحديث اللحظي لـ Firebase لنقل الطلب إلى الأرشيف
                $ticket_data['id'] = $ticket_id;
                $ticket_data['status'] = 'completed';
                $m_username = $_SESSION['username'] ?? $pdo->query("SELECT username FROM users WHERE id = $user_id")->fetchColumn();
                sync_to_firebase($m_username, 'archived_orders', $ticket_id, $ticket_data, 'PUT');
                sync_to_firebase($m_username, 'active_orders', $ticket_id, null, 'DELETE');
